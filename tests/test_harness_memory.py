...
@pytest.mark.parametrize("now", ['{"autoMemoryEnabled": true}', '{}'])
def test_on_never_restores_over_a_newer_choice(home, now):
    hm.turn_off("claude-code")
    ...
    assert asked and settings(home).read_text() == now and "kept" in out
