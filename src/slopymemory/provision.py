...
class SystemPostgres:
    ...
    def vector_installed(self, name: str) -> bool:
        """Is the vector extension installed IN this database (not merely available on the server)?"""
        name = self._ident(name)
        return self._psql("select 1 from pg_extension where extname = 'vector'", db=name) == "1"
